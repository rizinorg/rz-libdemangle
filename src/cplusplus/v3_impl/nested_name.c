...
DEFN_RULE (nested_name, {
    ...
    MATCH_AND_DO (
        READ ('N') && OPTIONAL (RULE (cv_qualifiers)) &&
            ...
            ((is_ctor = !!RULE (ctor_name)) || (is_dtor = !!RULE(dtor_name)) || RULE_DEFER(uname, unqualified_name)) &&
            READ ('E'),
        {
            if(is_ctor || is_dtor) {
                ...
                dem_string_append_n (dem, cname, pfx->buf + pfx->len - cname);
            } else {
                dem_string_concat(dem, pfx);
                APPEND_TYPE(dem);
                dem_string_append_n(dem, "::", 2);
    ...
    if(m->trace) {
        printf("[TRACE] -- nested-name -- Parsing with <prefix> failed in nested-name, now trying with <template-prefix>\n");
    }
