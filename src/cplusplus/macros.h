 ...
 */
#define RULE(x) (first_of_rule_##x (CUR()) ? rule_##x (dem, msi, m, graph, _my_node_id) : NULL)
#define RULE_CALL(x) rule_##x (dem, msi, m, graph, _my_node_id)
...
#define RULE_DEFER(var, x)                                                                         \
    (first_of_rule_##x (CUR()) ? rule_##x ((var), msi, m, graph, _my_node_id) : NULL)
...
#define RULE_ATLEAST_ONCE(x)                                                                       \
    match_one_or_more_rules (first_of_rule_##x, rule_##x, NULL, dem, msi, m, graph, _my_node_id)
#define RULE_ATLEAST_ONCE_WITH_SEP(x, sep)                                                         \
    match_one_or_more_rules (first_of_rule_##x, rule_##x, sep, dem, msi, m, graph, _my_node_id)
 ...
 */
#define RULE_MANY(x)                                                                               \
    match_zero_or_more_rules (first_of_rule_##x, rule_##x, NULL, dem, msi, m, graph, _my_node_id)
#define RULE_MANY_WITH_SEP(x, sep)                                                                 \
    match_zero_or_more_rules (first_of_rule_##x, rule_##x, sep, dem, msi, m, graph, _my_node_id)
...
#define TRACE_RETURN_SUCCESS(result)                                                               \
    do { /*trace success*/                                                                         \
        if (graph && graph->enabled && _my_node_id >= 0) {                                         \
        ...
        if ((rules)) {                                                                             \
            ...
            if (graph && graph->enabled && _my_node_id >= 0) {                                     \
                trace_graph_set_result (                                                           \
                    graph,                                                                         \
                    _my_node_id,                                                                   \
                    (_tmp_dem.buf && _tmp_dem.len > 0) ? _tmp_dem.buf : "success",                 \
                    1                                                                              \
                );                                                                                 \
            }                                                                                      \
                                                                                                   \
