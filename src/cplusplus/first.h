...
static inline bool first_of_rule_special_name(const char *input) {
	return input[0] == 'T' || strncmp(input, "TV", 2) == 0 || strncmp(input, "TT", 2) == 0 ||
		strncmp(input, "TI", 2) == 0 || strncmp(input, "TS", 2) == 0 ||
		strncmp(input, "Tc", 2) == 0 || strncmp(input, "GV", 2) == 0 ||
		strncmp(input, "GR", 2) == 0 || strncmp(input, "GTt", 3) == 0;
}
...
// nested-name-with-substitution-only rule
static inline bool first_of_rule_nested_name_with_substitution_only(const char *input) {
	return strncmp(input, "NS", 2) == 0 || strncmp(input, "NrS", 3) == 0 ||
		strncmp(input, "NrKS", 4) == 0 || strncmp(input, "NrVKS", 5) == 0 ||
		strncmp(input, "NrVS", 4) == 0 || strncmp(input, "NVKS", 4) == 0 ||
		strncmp(input, "NVS", 3) == 0 || strncmp(input, "NKS", 3) == 0

		|| strncmp(input, "NRS", 3) == 0 || strncmp(input, "NrRS", 4) == 0 ||
		strncmp(input, "NrKRS", 5) == 0 || strncmp(input, "NrVKRS", 6) == 0 ||
		strncmp(input, "NrVRS", 5) == 0 || strncmp(input, "NVKRS", 5) == 0 ||
		strncmp(input, "NVRS", 4) == 0 || strncmp(input, "NKRS", 4) == 0

		|| strncmp(input, "NOS", 3) == 0 || strncmp(input, "NrOS", 4) == 0 ||
		strncmp(input, "NrKOS", 5) == 0 || strncmp(input, "NrVKOS", 6) == 0 ||
		strncmp(input, "NrVOS", 5) == 0 || strncmp(input, "NVKOS", 5) == 0 ||
		strncmp(input, "NVOS", 4) == 0 || strncmp(input, "NKOS", 4) == 0;
}
...
static inline bool first_of_rule_exception_spec(const char *input) {
	return strncmp(input, "Do", 2) == 0 || strncmp(input, "DO", 2) == 0 ||
		strncmp(input, "Dw", 2) == 0;
}
...
static inline bool first_of_rule_function_param(const char *input) {
	return strncmp(input, "fp", 2) == 0 || strncmp(input, "fL", 2) == 0 ||
		strncmp(input, "fpT", 3) == 0 || first_of_rule_number(input);
}
...
static inline bool first_of_rule_base_unresolved_name(const char *input) {
	return strncmp(input, "on", 2) == 0 || strncmp(input, "dn", 2) == 0 ||
		first_of_rule_number(input);
}

// unresolved-name rule
static inline bool first_of_rule_unresolved_name(const char *input) {
	return strncmp(input, "sr", 2) == 0 || strncmp(input, "srN", 3) == 0 ||
		strncmp(input, "gs", 2) == 0 || first_of_rule_base_unresolved_name(input);
}
...
static inline bool first_of_rule_class_enum_type(const char *input) {
	...
	return first_of_rule_number(input) || first_of_rule_operator_name(input) ||
		first_of_rule_ctor_dtor_name(input) || first_of_rule_unnamed_type_name(input) ||
...
static inline bool first_of_rule_type(const char *input) {
	...
	return first_of_rule_builtin_type(input) ||
		first_of_rule_extended_qualifier(
			input) || // This covers both single and {extended-qualifier}+ patterns
		first_of_rule_function_type(input) ||
		first_of_rule_class_enum_type(input) || first_of_rule_array_type(input) ||
		first_of_rule_pointer_to_member_type(input) || first_of_rule_template_param(input) ||
		first_of_rule_template_template_param(input) || first_of_rule_decltype(input) ||
		first_of_rule_substitution(input);
}
...
static inline bool first_of_rule_name(const char *input) {
	return first_of_rule_nested_name(input) || first_of_rule_unscoped_name(input) ||
		first_of_rule_unscoped_template_name(
			input) || // {unscoped-template-name} {template-args} starts with unscoped-template-name
...
static inline bool first_of_rule_expression(const char *input) {
	...
	return first_of_rule_operator_name(input) || first_of_rule_template_param(input) ||
		first_of_rule_function_param(input) || first_of_rule_unresolved_name(input) ||
...
static inline bool first_of_rule_braced_expression(const char *input) {
	return strncmp(input, "di", 2) == 0 || strncmp(input, "dx", 2) == 0 ||
		strncmp(input, "dX", 2) == 0 || first_of_rule_expression(input);
}
...
static inline bool first_of_rule_unresolved_type(const char *input) {
	...
	// {decltype}
	// {substitution}
	return first_of_rule_template_param(input) || first_of_rule_decltype(input) ||
		first_of_rule_substitution(input);
}
...
static inline bool first_of_rule_prefix_or_template_prefix_start(const char *input) {
	...
	return first_of_rule_unqualified_name(input) || first_of_rule_template_param(input) ||
		first_of_rule_substitution(input);
}
...
static inline bool first_of_rule_closure_prefix_unit(const char *input) {
	...
	return first_of_rule_prefix_or_template_prefix_start(input) ||
		first_of_rule_decltype(input) || first_of_rule_unqualified_name(input);
}
...
static inline bool first_of_rule_closure_prefix_rr(const char *input) {
	...
	return first_of_rule_unqualified_name(input) || first_of_rule_prefix_start_rr(input);
}
...
static inline bool first_of_rule_prefix_start_unit(const char *input) {
	...
	// {closure-prefix}
	return first_of_rule_prefix_or_template_prefix_start(input) || first_of_rule_decltype(input);
