 ...
 * \return NULL otherwise.
 */
typedef DemString* (*DemRule) (DemString* dem, StrIter* msi, StrVec* detected_types);
 ...
 * \return NULL otherwise.
 */
static DemString* match_one_or_more_rules (
    DemRule     rule,
    const char* sep,
    DemString*  dem,
    StrIter*    msi,
    ...
    if (!rule || !dem || !msi || !detected_types) {
        return NULL;
    }
    ...
    if (rule (dem, msi, detected_types)) {
        ...
        while (true) {
            DemString tmp = {0};
            SAVE_POS();
            if (rule (&tmp, msi, detected_types)) {
 ...
 */
#define RULE(x) rule_##x (dem, msi, detected_types)
 ...
 * \return NULL if rule match fails for any reason.
 */
#define RULE_DEFER(var, x) rule_##x ((var), msi, detected_types)
#define DEFER_VAR(var)                                                                             \
    DemString  tmp_defer_var_##x = {};                                                             \
    DemString* var               = &tmp_defer_var_##x;                                             \
    dem_string_init (var);
#define APPEND_DEFER_VAR(var) (APPEND_STR ((tmp_defer_var_##x).buf), dem_string_deinit (var), 1)
 ...
 * \return NULL if rule match fails for any reason.
 */
#define RULE_ATLEAST_ONCE(x) match_one_or_more_rules (rule_##x, NULL, dem, msi, detected_types)
#define RULE_ATLEAST_ONCE_WITH_SEP(x, sep)                                                         \
    match_one_or_more_rules (rule_##x, sep, dem, msi, detected_types)
 ...
 * \return NULL if rule match fails for any reason.
 */
#define RULE_MANY(x) match_zero_or_more_rules (rule_##x, NULL, dem, msi, detected_types)
#define RULE_MANY_WITH_SEP(x, sep)                                                                 \
    match_zero_or_more_rules (rule_##x, sep, dem, msi, detected_types)
 ...
 */
#define DECL_RULE(x)                                                                               \
    static DemString* rule_##x (DemString* dem, StrIter* msi, StrVec* detected_types)
...
#define APPEND_TYPE(tname)                                                                         \
    (vec_append (detected_types, ((StrVec[]) {{0}})),                                              \
     dem_string_init_clone (vec_end (detected_types), (tname)))
...
#define SUBSTITUTE_TYPE(id)                                                                        \
    (detected_types->length > (id) ?                                                               \
         (APPEND_STR (vec_ptr_at (detected_types, (id))->buf) ? dem : NULL) :                      \
         NULL)
...
DEFN_RULE (encoding, {
    MATCH (
        RULE (function_name) && APPEND_CHR ('(') && RULE (bare_function_type) && APPEND_CHR (')')
    );
...
DEFN_RULE (nested_name, {
     ...
     */
    MATCH (
        READ ('N') && OPTIONAL (RULE (cv_qualifiers) && APPEND_CHR (' ')) &&
        RULE_DEFER (ref, ref_qualifier) && RULE (prefix) && RULE (unqualified_name) && READ ('E') &&
        APPEND_DEFER_VAR (ref)
    );
    MATCH (
        READ ('N') && OPTIONAL (RULE (cv_qualifiers) && APPEND_CHR (' ')) && RULE (prefix) &&
        RULE (unqualified_name) && READ ('E')
    );
    ...
    MATCH (
        READ ('N') && OPTIONAL (RULE (cv_qualifiers) && APPEND_CHR (' ')) &&
        RULE_DEFER (ref, ref_qualifier) && RULE (template_prefix) && RULE (template_args) &&
        READ ('E') && APPEND_DEFER_VAR (ref)
    );
    MATCH (
        READ ('N') && OPTIONAL (RULE (cv_qualifiers) && APPEND_CHR (' ')) &&
        RULE (template_prefix) && RULE (template_args) && READ ('E')
    );
});
...
DEFN_RULE (function_type, {
    MATCH (
        OPTIONAL (RULE (cv_qualifiers) && APPEND_CHR (' ')) && OPTIONAL (RULE (exception_spec)) &&
        READ_STR_OPTIONAL ("Dx") && READ ('F') && READ_OPTIONAL ('Y') &&
        RULE (bare_function_type) && OPTIONAL (RULE (ref_qualifier)) && READ ('E')
    );
});
...
DEFN_RULE (qualifiers, {
    MATCH (RULE_MANY (extended_qualifier) && RULE (cv_qualifiers) && APPEND_CHR (' '));
});
